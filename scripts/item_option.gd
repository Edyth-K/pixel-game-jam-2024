extends ColorRect

var mouse_over = false
var item = null
@onready var player = get_tree().get_first_node_in_group("player")
@onready var label_name = $LabelName
@onready var label_level = $LabelLevel
@onready var label_description = $LabelDescription
@onready var item_icon = $IconBG/ItemIcon

signal selected_upgrade(upgrade)

func _ready():
	connect("selected_upgrade", Callable(player, "upgrade_character"))
	if item == null:
		item = "heal"
	label_name.text = UpgradeDb.UPGRADES[item]["name"]
	label_level.text = UpgradeDb.UPGRADES[item]["level"]
	label_description.text = UpgradeDb.UPGRADES[item]["description"]
	item_icon.texture = load(UpgradeDb.UPGRADES[item]["icon"])

func _input(event):
	if event.is_action("click"):
		if mouse_over:
			emit_signal("selected_upgrade", item)

#pseudo button functionality
func _on_mouse_entered():
	mouse_over = true


func _on_mouse_exited():
	mouse_over = false
